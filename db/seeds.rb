...
File.new("db/players.txt", 'r').each_line("\n") do |row|
  ...
  Player.create!(:display_name => display_name.strip, :short_name => short_name, :role => role, :country_id => country_id)
end
...
File.new("db/stadia.txt", 'r').each_line("\n") do |s|
  ...
  Stadium.create!(:name => name.strip, :city => city)
end
...
File.new("db/matches.txt", 'r').each_line("\n") do |m|
  ...
  Match.create!(:date => Date.parse(date), :country_one_id => country_one_id, :country_two_id => country_two_id, :match_type => match_type, :stadium_id => stadium_id, :day_night => day_night.strip)
end
